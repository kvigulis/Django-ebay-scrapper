...
def loadEBayResults(printWhileRunning, url):
    ...
    #display.start()
    first_window = webdriver.Chrome("C:/chromedriver")  # Search result window (main)
    first_window.implicitly_wait(10)
    ...
    for x in range(len(html_items)):
        ...
        price = float(non_decimal.sub('', str(priceDirty.rsplit(' ', 1)[0])))
    ...
    first_window.close()
    #display.stop()
    # TODO: Consider returning a list or a dictionary to get rid of too many funtions in searchSetups
